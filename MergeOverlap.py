...
def directory_check(input_path) -> None:
    """
    Creates necessary directories for merge output.
    """

    aa_merged = os.path.join(input_path,'aa_merged')
    nt_merged = os.path.join(input_path,'nt_merged')
...
def main(gene, output_dir, aa_path, nt_path, fallback_taxa, debug, majority, minimum_mr_amount) -> None:
...
def run_command(arg_tuple: tuple) -> None:
    gene,output_dir,aa_path,nt_path,comparison,debug,majority,majority_count = arg_tuple
    main(gene,output_dir,aa_path,nt_path, comparison,debug,majority,majority_count)
        ...
        if os.path.exists(aa_input):
            ...
            if args.processes:
                ...
                for gene in file_inputs:
                    ...
                    arguments.append((gene,taxa_path,aa_path,nt_path,args.comparison,args.debug,args.majority,args.majority_count, ))
                ...
                for gene in file_inputs:
                    ...
                    nt_gene = make_nt_name(gene)
                    nt_path = os.path.join(nt_input,nt_gene)
                    main(gene,taxa_path,aa_path,nt_path,args.comparison,args.debug,args.majority,args.majority_count)
