...
def deinterleave(fasta_lines: list) -> str:
    ...
    for line in fasta_lines:
        if '>' in line:
            this_header = line
            de_interleaved[this_header] = ''
        else:
            if line != '':
                de_interleaved[this_header] += line.replace('\n','')

    for header in de_interleaved:
        result.append(header)
        result.append(de_interleaved[header])
...
if __name__ == '__main__':
    ...
    file_inputs = [gene for gene in os.listdir(args.aa) if '.aa' in gene]

    if args.processes:
        arguments = list()
        for gene in file_inputs:
            arguments.append((args.aa,args.nt,args.output,args.matches,gene,tmp_path))
