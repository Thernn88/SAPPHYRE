...
def calculate_split(sequence_a: str, sequence_b: str, comparison_sequence: str) -> int:
    ...
    start_a,end_a = get_start_end(sequence_a)
    start_b,end_b = get_start_end(sequence_b)
    ...
    comparison_sequence = comparison_sequence[overlap_start:overlap_end+1]

    highest_score = 0
    highest_scoring_pos = None
...
def directory_check() -> None:
    """
    Creates necessary directories for merge output.
    """
    script_dir = os.getcwd()

    aa_merged = os.path.join(script_dir,'aa_merged')
    nt_merged = os.path.join(script_dir,'nt_merged')
...
def main(gene,aa_path,nt_path,fallback_taxa,debug,majority,minimum_mr_amount) -> None:
    ...
    for protein in ['aa','nt']:
        ...
        for taxa in taxa_to_sequences:
            this_sequences = taxa_to_sequences[taxa]
            this_sequences.sort(key=lambda x: x[0])
            ...
            while cursor < trailing_end:
                ...
                if len(sequences_at_current_point) == 1:
                    new_merge += sequences_at_current_point[0][1][cursor]
                    ...
                    if most_occuring[0][1] == 1: #No taxa occurs more than once
                        comparison_taxa = fallback_taxa
                    else:
                        comparison_taxa = most_occuring[0][0]
                    ...
                    if len(candidate_characters) >= minimum_mr_amount:
                        ...
                        if perc_appearing >= majority:
                            if mode_cand_character != char:
                                ...
                                majority_assignments += mode_cand_character
                                mr_won = True
                    if not mr_won:
                    ...
                    if len(candidate_characters) >= minimum_mr_amount:
                        ...
                        if perc_appearing >= majority:
                            if mode_cand_raw_character != char:
                                ...
                                majority_assignments += mode_cand_raw_character
            ...
            if debug == True: # If debug enabled add each component under final merge
                ...
                gene_out.append(majority_assignments)
                for header,sequence in consists_of:
            ...
            if protein == 'aa':
                output_path = os.path.join(current_dir,'aa_merged',gene)
            else:
                output_path = os.path.join(current_dir,'nt_merged',gene)

        open(output_path,'w').write('\n'.join(gene_out))
    
def run_command(arg_tuple: tuple) -> None:
    gene,aa_path,nt_path,comparison,debug,majority,majority_count = arg_tuple
    main(gene,aa_path,nt_path, comparison,debug,majority,majority_count)
...
if __name__ == '__main__':
    ...
    file_inputs = []
    for item in os.listdir(args.aa_input):
        if '.fa' in item:
    ...
    if args.processes:
        ...
        for gene in file_inputs:
            aa_path = os.path.join(args.aa_input,gene)
            nt_gene = make_nt_name(gene)
            nt_path = os.path.join(args.nt_input,nt_gene)
            arguments.append((gene,aa_path,nt_path,args.comparison,args.debug,args.majority,args.majority_count, ))
        ...
        for gene in file_inputs:
            ...
            nt_gene = make_nt_name(gene)
            nt_path = os.path.join(args.nt_input,nt_gene)
            main(gene,aa_path,nt_path,args.comparison,args.debug,args.majority,args.majority_count)
