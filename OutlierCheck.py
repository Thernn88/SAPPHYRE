...
def deinterleave(fasta_lines: list) -> str:
    ...
    for line in fasta_lines:
        if '>' in line:
            if this_header != None:
                result.append(this_header)
                result.append(this_out+'\n')
            this_header = line
            this_out = ''
        else:
            if line != '':
                this_out += line.replace('\n','')
...
if __name__ == '__main__':
    ...
    aa_path = os.path.join(args.input,'clean','trimmed',args.aa_input)
    file_inputs = [os.path.join(aa_path,gene) for gene in os.listdir(aa_path) if '.aa' in gene and gene.split('.')[-1] in allowed_extensions]
    ...
    if args.processes:
        ...
        for gene in file_inputs:
            arguments.append((gene,output_path,args.threshold,args.no_references, args.sort))
        ...
        for gene in file_inputs:
            print(gene)
            main_process(gene,output_path,args.threshold,args.no_references, args.sort)
